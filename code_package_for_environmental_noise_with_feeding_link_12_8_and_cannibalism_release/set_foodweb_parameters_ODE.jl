...
module SetFoodWebParametersODE
...
vorts_data_file = "../FuncResp/vorts_data/abundances_imputed_3.csv"
vorts_catch_data_file = "../FuncResp/vorts_data/catches_truncated_2.csv"

# FOODWEB STRUCT 
mutable struct FoodWebDetritalLoopODE
        data_name::String
        ...
        # lower boundaries of the intrinsic growth rates of producers (r_i) and the metabolic rates of consumers (T_i)
        ...
        denom_sum
        G 
        producers_loss_to_consumers 
        consumers_gain_from_resources 
        consumers_loss_to_consumers 
        exudation 
        cons_egestion 
        cons_by_detritivores 
end



# INTIALIZE A FOODWEB STRUCT WITH PARAMETERIZATION CORRESPONDING TO VORTS
function initialize_vorts_foodweb_ODE_disc(model_type,q_init)
        ...
        time_ind = 1:length(vorts_data[1,:])
        ...
        I[10,2:3].=1 # Smelt
        I[11,2:10].=1 # Perch # cannibalism EXCLUDED
        #I[12,2:10].=1 # Eel DIFFERS FROM CREMONA ET AL. !!!!
        I[12,2:7].=1 # Eel
        I[12,9:10].=1 # Eel
        I[13,2:11].=1 #Pikeperch
        I[14,2:3].=1
        I[14,5:13].=1 # Pike # cannibalism EXCLUDED
        ...
        F = zeros(nGuilds,nGuilds)
        denom_sum = zeros(nGuilds)
        G = zeros(nGuilds)
        producers_loss_to_consumers = zeros(nGuilds)
        consumers_gain_from_resources = zeros(nGuilds)
        consumers_loss_to_consumers = zeros(nGuilds)
        exudation = 0.0
        cons_egestion = 0.0
        cons_by_detritivores = 0.0
    
        return vorts_data, FoodWebDetritalLoopODE(data_name,model_type,vorts_names,
        time, time_ind, 
        nGuilds,
        nLinks, nProd, nCons,
        I_ind,
        ...
        J, e, B0, q, 
        cv, std,
        F,denom_sum,G,producers_loss_to_consumers,consumers_gain_from_resources,
        consumers_loss_to_consumers,exudation,cons_egestion,cons_by_detritivores)

end


function copy_foodweb_ODE(fw_old)
        ...
        F = copy(fw_old.F)
        denom_sum = copy(fw_old.denom_sum)
        G = copy(fw_old.G)
        ...
        exudation = copy(fw_old.exudation)
        cons_egestion = copy(fw_old.cons_egestion)
        cons_by_detritivores = copy(fw_old.cons_by_detritivores)

        return FoodWebDetritalLoopODE(data_name,model_type,guild_names,
                time, time_ind, 
                nGuilds,
                nLinks,nProd,nCons,
                I_ind,
